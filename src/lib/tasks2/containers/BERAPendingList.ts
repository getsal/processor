...
export default class BERAPendingList {
    ...
    constructor() {
        ...
        this._filePath = path.join(config.dataDir, 'BERA-pendingTransactions.bin');
        ...
        setInterval(async () => {
            ...
            const collection = database.collection('transactions_BERA');
        ...
        try {
            ...
            const obtainBlock = async (): Promise<any> => {
                try {
                    ...
                    const filePath = path.join('blocks', 'BERA', hash);
            ...
            if (!block) {
                console.log(`BERAPendingList Failed to get data for block: ${hash}`);
                return;
            }
